                    ...
                    if (line.startsWith('data: ')) {
                        ...
                        try {
                            ...
                            if (eventType === 'text_chunk') {
                                ...
                                // 🆕 顯示每輪時間明細（Backend Operator - 從 done.round_details 讀取）
                                if (eventPayload.timing && eventPayload.timing.round_details) {
                                    const roundDetails = eventPayload.timing.round_details;
                                    statsHtml += `📊 <b>每輪時間</b>: `;
                                    roundDetails.forEach((r, idx) => {
                                        if (idx > 0) statsHtml += ' | ';
                                        statsHtml += `第${r.round}輪:LLM ${r.llm_ms}ms`;
                                        if (r.tool_ms) statsHtml += `+Tool ${r.tool_ms}ms`;
                                    });
                                    statsHtml += `<br>`;
                                }
                                ...
                                if (eventPayload.timing) {  // ✅ v2.0: timings → timing
                                    ...
                                    statsHtml += `📊 <b>總計</b>: `;
                                    statsHtml += `輪數:${t.rounds||0} | Tools:${t.tools_executed||0} | 總時間:${t.total_ms||0}ms`;
                                }
